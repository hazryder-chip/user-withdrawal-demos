...
final readonly class FetchWithdrawalsByRecordCountHandler
{
    public function __construct(
        private FetchWithdrawalsByRecordCountQuery $query
    ) {
    }
    ...
    public function __invoke(Request $request, string $chipUserId, string $startDate, string $endDate): JsonResponse
    {
        ...
        if ($useCache) {
            $result = Cache::get('withdrawals_' . $chipUserId);
            ...
            if(!isset($result)) {
                ...
                Cache::put('withdrawals_' . $chipUserId, $result);
            }
        } else {
            $result = $this->query->execute(
                $chipUserId,
